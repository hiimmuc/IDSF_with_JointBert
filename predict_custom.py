...
class JointBertTools:
    ...
    def __init__(self, model_dir, batch_size, no_cuda=True):
    ...
    def load_model(self):
        ...
        try:
            model = MODEL_CLASSES[self.args.model_type][1].from_pretrained(self.args.model_dir,
                                                                           args=self.args,
                                                                           intent_label_lst=get_intent_labels(
                                                                               self.args),
                                                                           slot_label_lst=get_slot_labels(self.args))
...
if __name__ == '__main__':
    ...
    text = ['let me know the weather in Hanoi',
            'Is it windy in Boston, MA right now?',
            'I want to hear Faded Alan Walker',
            'let play the song Hello aldele for me']
    # print([word for word in text.strip().split()])
    output_text = predict_tools.predict(text)
